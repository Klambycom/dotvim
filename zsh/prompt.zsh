...
GIT_PROMPT_STAGED="%{$fg_bold[green]%}staged%{$reset_color%}"
...
setopt prompt_subst
PROMPT='%{$fg[green]%}%1d%{$reset_color%} $(git_prompt_string) %{$fg[green]%}$ %{$reset_color%}'


#PROMPT='
#${PR_GREEN}%n%{$reset_color%} %{$FG[239]%}at%{$reset_color%} %{$FG[239]%}in%{$reset_color%} ${PR_BOLD_YELLOW}$(current_pwd)%{$reset_color%} 
#$(prompt_char) '
